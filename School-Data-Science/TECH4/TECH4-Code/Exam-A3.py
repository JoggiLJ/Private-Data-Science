...
# To solve this problem, we only need to add this constraint ensuring that there is only 1 electrical station.
model.constraints.add(sum(model.s[r, c] for r in range(rows) for c in range(cols)) == 1)
...
print(f'\nSolution to the {model.name} problem | Task A3')
